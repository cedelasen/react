...
def vorFiniteDelPolygonsList(vor, box):
    ...
    mins = numpy.tile((min_x, min_y), (vertices.shape[0], 1))
    bounded_vertices = numpy.max((vertices, mins), axis=0)
    maxs = numpy.tile((max_x, max_y), (vertices.shape[0], 1))
    bounded_vertices = numpy.min((bounded_vertices, maxs), axis=0)
